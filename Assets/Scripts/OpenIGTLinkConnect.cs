...
public class OpenIGTLinkConnect : MonoBehaviour
{
    ...
    //public string ipString = "192.168.0.120";
    ...
    private void ConnectCallback(IAsyncResult ar)
    {
        try
        {
            ...
            //Debug.Log("Connected");
            //debugText.GetComponent<TextMeshPro>().SetText("Connected");
            //connectText.GetComponentInChildren<TextMeshPro>().SetText("Connected");
    ...
    private void ReceiveCallback(IAsyncResult ar)
    {
        try
        {
            //debugText.GetComponent<TextMeshPro>().SetText("Reading Bytes");
    ...
    IEnumerator ReceiveTransformMessage(byte[] data, string transformName)
    {
        ...
        foreach (GameObject gameObject in GameObjects)
        {
            ...
            if (objectName.Equals(transformName) & gameObject.GetComponent<OpenIGTLinkFlag>().ReceiveTransform)
            {
                ...
                IJKToRAS.SetRow(1, new Vector4(0, -1.0f, 0, 0));
                ...
                Vector3 translation = matrixRAS.GetColumn(3);
